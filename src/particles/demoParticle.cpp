...
void demoParticle::update(){
    ...
    if( mode == PARTICLE_MODE_ATTRACT ){
        ...
        float distance = frc.length();
        float distInit = frc2Init.length();
        frc.normalize();
        frc2Init.normalize();
        ...
        if (ofGetKeyPressed(' ')) {
            ...
            if (distInit < resetDistThreshold){
                ...
                vel += -frc2Init * returnForce;
            }
        } else {
            if( distance < dist ){
                vel += -frc * force; //notice the frc is negative
            } else {
                // avoid oscillating around init point
                if (distInit < resetDistThreshold){
                    vel = ofPoint(0, 0);
